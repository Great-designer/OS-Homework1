...
//初始化线程。
	...
	t->ticks_blocked=0;//在线程被创建的时候初始化ticks_blocked为0。 
/* Stack frame for kernel_thread(). */
//kernel_thread（）的堆栈帧。
...
void thread_unblock(struct thread *t) 
{
	...
	list_insert_ordered(&ready_list,&t->elem,(list_less_func*)&thread_cmp_priority,NULL);//list_push_back(&ready_list,&t->elem);
...
void thread_yield(void)//把当前线程扔到就绪队列里， 然后重新schedule， 注意这里如果ready队列为空的话当前线程会继续在cpu执行。
{
	...
	{
		list_insert_ordered(&ready_list,&cur->elem,(list_less_func *)&thread_cmp_priority,NULL);//list_push_back(&ready_list,&cur->elem);//调用list_push_back把当前线程的元素扔到就绪队列里面。
	}
...
static void init_thread(struct thread *t,const char *name,int priority)
{
	...
	list_insert_ordered(&all_list,&t->allelem,(list_less_func *)&thread_cmp_priority,NULL);//list_push_back(&all_list,&t->allelem);
	intr_set_level(old_level);//原子操作。
}
